...
public:
     ...
     */
    explicit Subscriber(std::function<void (std::string_view)> callback, std::string channel, int32_t stream_id = 1001);
    ...
    * @param channel Канал Aeron. В общем случае для указания канала используется URI
    */
    void addDestination(std::string channel);
    ...
    * @param channel Канал Aeron. В общем случае для указания канала используется URI
    */
    void removeDestination(std::string channel);
