 ...
 */
Subscriber::Subscriber(std::function<void (std::string_view)> callback, std::string channel, int32_t stream_id)
 ...
 * @param channel Канал Aeron. В общем случае для указания канала используется URI
 */
void Subscriber::addDestination(std::string channel)
{
    subscription->addDestination(channel);
}
 ...
 * @param channel Канал Aeron. В общем случае для указания канала используется URI
 */
void Subscriber::removeDestination(std::string channel)
{
    subscription->removeDestination(channel);
}
